...
class FileHandler:
    ...
    def read_file(self, file_path: str, mode: str = 'r'):
        ...
        """
        logger.log_to_stderr(f"Reading file {file_path} as '{mode}' ") if self.verbose else None
        try:
    ...
    def write_file(self, file_path: str, content, mode: str = 'w') -> None:
        """
        ...
        """
        logger.log_to_stderr(f"Writing in '{mode}' to file {file_path}") if self.verbose else None
        try:
            with open(file_path, mode=mode) as file:
                file.write(content)
        except FileNotFoundError as e:
    ...
    def hash_file(self, file_path: str = None, _type: str = 'md5') -> str:
        """
        Hash given file
        ...
        """
        if not file_path:
            logger.log_to_stderr(f'File path not provided', level='ERROR') if self.verbose else None
            raise errors.FileError('File path not provided')
        if _type not in ['md5', 'sha1']:
            logger.log_to_stderr(f'Unsupported hash type: {_type}', level='ERROR') if self.verbose else None
        ...
        logger.log_to_stderr(f'Hashing file {file_path} using {_type}') if self.verbose else None
        if _type == 'md5':
...
class CHelper:
    ...
    def compare_mac(self, secret: bytes, data: bytes, mac: bytes) -> bool:
        """
            Compare the MAC of the data with given MAC (MAC = Message Authentication Code)
    ...
    def generate_rsa_key_pair(self, size: int, file_name: str = None, test: bool = False) -> bool:
        ...
        try:
            FileHandler(self.verbose).write_file(f'keys\\{file_name}private.pem', private_key.private_bytes(
                encoding=serialization.Encoding.PEM,
                format=serialization.PrivateFormat.TraditionalOpenSSL,
                encryption_algorithm=serialization.NoEncryption()
            ).decode('utf-8'))

            FileHandler(self.verbose).write_file(f'keys\\{file_name}public.pem', public_key.public_bytes(
                encoding=serialization.Encoding.PEM,
    ...
    def generate_aes_key(self, size: int = 32, file_name: str = None, test: bool = False) -> bytes:
        ...
        FileHandler(self.verbose).write_file(f'keys\\{file_name}aes_key.txt', str(_key))
    ...
    def generate_twofish_key(self, size: int = 32, file_name: str = None, test: bool = False) -> bytes:
        ...
        _key = secrets.token_bytes(size)
        FileHandler(self.verbose).write_file(f'keys\\{file_name}twofish_key.txt', str(_key))
        return _key

    def get_key(self, key_path: str) -> bytes:
        """
        Get the key from given path
        ...
        if key_path.endswith('.txt'):
            ...
            return ast.literal_eval(FileHandler(self.verbose).read_file(key_path))
        elif key_path.endswith('.pem'):
            logger.log_to_stderr(f'Reading key [RSA] from {key_path}') if self.verbose else None
            return FileHandler(self.verbose).read_file(key_path).encode('utf-8')


class EncryptionHandler:
    """
    Class to handle encryption, should work like so:
    - Initialize class with path to key, and context
    ...
    def encrypt_text(self, text: str, _is_file: bool = False, _type: str = None) -> bytes:
        """
        Encrypts given text or gets the text from file and encrypts it
        ...
        if _is_file:
            text = FileHandler(self.verbose).read_file(text)
    ...
    def _encrypt_aes(self, context: str) -> tuple:
        """
        Encrypt given context using AES in CTR mode
...
class DecryptionHandler:
    ...
    def decrypt_text(self, text: bytes | str, _is_file: bool = False, _type: str = None) -> str:
        """
            Decrypts given text or gets the text from the file and decrypts it
                :param text:
                :param _is_file:
                :param _type:
        ...
        _ = text if not _is_file else FileHandler(self.verbose).read_file(text, 'rb')
    ...
    def _decrypt_aes(self, iv, enc_context):
        """
        Decrypt given context using AES in CTR mode
