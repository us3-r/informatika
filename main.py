...
arg_parser.add_argument('-hf', '--hash-file', type=str, metavar='path/to/file',
                        help='[*] Hash file')

# flags for running style
...
if _enc:
    ...
    if _file:
        enc_data = e.encrypt_text(_file, True, _enc.split(',')[0].upper())
        print((
                "[ENCRYPTED TEXT]\n" +
                "+---------------\n" +
                f"{enc_data}" +
                "\n+---------------"
        )) if not _output_file else _filehandler.write_file(_output_file, enc_data, 'wb')
    elif _text:
...
if _dec:
    d = DecryptionHandler(
        key_path=_private_key_path if _dec.split(',')[0].upper() == 'RSA' else _key,
    ...
    if _file:
        ...
        print((
                ...
                f"{dec_data}" +
                "\n+---------------"
        )) if not _output_file else _filehandler.write_file(_output_file, str(dec_data))
...
if _to_hash_file:
    file_hash = _filehandler.hash_file(_to_hash_file)
    print((
            "[FILE HASH]\n" +
            "+---------------\n" +
            f"{file_hash}" +
            "\n+---------------"
    )) if not _output_file else _filehandler.write_file(_output_file, file_hash)
